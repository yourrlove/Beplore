...
import repliesAtom from "../atoms/repliesAtom";
  ...
  const Comment = ({ reply, postedBy }) => {
    const currentUser = useRecoilValue(userAtom);
    const [replies, setReplies] = useRecoilState(repliesAtom);
    const { onOpen, onClose, isOpen } = useDisclosure();
    const [liked, setLiked] = useState(reply?.likes.includes(currentUser?._id));
    const [loading, setLoading] = useState(false);
    const showToast = useShowToast();
    const navigate = useNavigate();
    const formatDate = () => {
      const result = formatDistanceToNowStrict(new Date(reply.createdAt)).split(
        " "
      );
      if (result[0] > 7 && result[1] === "days") {
        return format(new Date(reply.createdAt), "MM/dd/yyyy");
      }
    ...
    const handleLike = async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/posts/comments/${reply._id}/likes`, {
          method: "PUT",
        });
        ...
        if (!liked) {
          const updatedComments = replies.map((p) => {
            if (p._id === reply._id) {
    ...
    const handleDelete = async (e) => {
      ...
      try {
        ...
        const pathComments = reply.parentComment.split(',');
        const parentComment = pathComments.pop();
        const res = await fetch(`/api/posts/${parentComment}/comments/${comment._id}`, {
          method: "DELETE",
        });
        const result = await res.json();
        if (result.status === "error") {
